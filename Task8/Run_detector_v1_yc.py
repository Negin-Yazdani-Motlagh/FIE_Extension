"""
Task 8: Run Detector v1 on YC Corpus
Goal: Apply lexicon v1 to YC corpus (2012-2024) to generate baseline detection
"""
...
# File paths
YC_CORPUS_FILE = TASK8_DIR / "yc_corpus_2012_2024.parquet"
LEXICON_V1_FILE = TASK3_DIR / "softskill_tag_mapping.csv"  # Lexicon v1

OUTPUT_FILE = TASK8_DIR / "yc_detector_v1_results.parquet"
SUMMARY_FILE = TASK8_DIR / "yc_detector_v1_summary.json"
LOG_FILE = TASK8_DIR / "task8_detector_v1_log.json"
...
def load_lexicon_v1():
    """Load lexicon v1 from Task 3"""
    print("\n1. Loading lexicon v1...")
    print(f"   File: {LEXICON_V1_FILE.name}")
    
    df = pd.read_csv(LEXICON_V1_FILE)
    ...
    print(f"   [OK] Loaded lexicon v1 with {len(soft_skill_domains)} domains")
...
def main():
    ...
    print("=" * 80)
    print("TASK 8: Run Detector v1 on YC Corpus")
    print("=" * 80)
    
    try:
        # Step 1: Load lexicon v1
        soft_skill_domains = load_lexicon_v1()
        ...
        # Step 3: Load YC corpus
        print("\n3. Loading YC corpus...")
        print(f"   File: {YC_CORPUS_FILE.name}")
        df = pd.read_parquet(YC_CORPUS_FILE)
        print(f"   [OK] Loaded {len(df):,} postings")
        
        # Step 4: Apply detector v1
        print("\n4. Applying detector v1...")
        domain_names = sorted(soft_skill_domains.keys())
        
        # Initialize columns
        for domain in domain_names:
            df[f'text_domain_present_{domain}_v1'] = False
            df[f'text_domain_count_{domain}_v1'] = 0
        ...
        for idx, row in df.iterrows():
            ...
            for domain in domain_names:
                if domain in results:
                    df.at[idx, f'text_domain_present_{domain}_v1'] = results[domain]['present']
                    df.at[idx, f'text_domain_count_{domain}_v1'] = results[domain]['count']
        ...
        # Overall statistics
        for domain in domain_names:
            present_col = f'text_domain_present_{domain}_v1'
            count_col = f'text_domain_count_{domain}_v1'
            ...
            for year in sorted(df['year'].dropna().unique()):
                ...
                for domain in domain_names:
                    ...
                    if present_col in year_df.columns:
                        ...
                        year_stats[domain] = {
                            'postings_with_domain': int(n_present),
                            'coverage_percent': round(100 * n_present / len(year_df), 1) if len(year_df) > 0 else 0
                        }
                
                summary['by_year'][int(year)] = year_stats
        ...
        print("\n" + "=" * 80)
        print("DETECTOR V1 COMPLETED SUCCESSFULLY")
        print("=" * 80)
        print(f"\nSummary:")
